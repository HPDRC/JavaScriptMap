            ...
            if (!!route) {
                if (route.completed) {
                    if (route.failed) {
                        ...
                        //if (!!goodRoute) { console.log('got good route'); }
...
tf.apps.HotelDemo.MovingPoint = function (settings) {
    ...
    function startRoute(routeSet) {
        if (!isDeleted) {
            ...
            //console.log(label + ' started route #' + nRoutes + ' from ' + labelStart + ' (' + startIndex + ') to ' + labelEnd + ' (' + endIndex + ') by ' + mode + ' distance(m): ' + distance + ' duration(s): ' + loopTime + ' speed(mph): ' + speed.toFixed(1));
...
tf.apps.HotelDemo.Shuttle = function (settings) {
    ...
    function makeStop(stopIndex, isStop) {
        /*var verb = isStop ? "stopped at" : "started from"
        console.log('shuttle ' + (index  + 1) + ' ' + verb + ' ' + stopIndex);*/
...
tf.apps.HotelDemo.Demo = function (settings) {
    ...
    function makeStops(viaIndices, coords) {
        ...
        for (var i in viaIndices) {
            ...
            if (!label.length) { console.log('stop destination not found: ' + i); }
    ...
    function onShuttleStartedRoute(notification) {
        onUpdateShuttleTableRow(notification);
        //console.log(notification.sender.GetNRoutes());
    }
                ...
                onClick: function () {
                    ...
                    if (videoDivElem.style.display == 'none') {
                        ...
                        if (mode == 'car') {
                            ...
                            //props.liveStreamPlayer.PlayStream("http://localhost/tests/a.flv");
                        }
                    }
                    else { stopVideo(mp); }
                }
    ...
    function initialize() {
        ...
        var defaultDLayers = {
            ...
            "dlayerlegend1": "Wikipedia",
            "dlayerdata1": "http://n00.cs.fiu.edu/cgi-bin/arquery.cgi?category=wikix2015&vid=&tfaction=shortdisplayflash&filetype=.xml",
            "dlayerselect1": "true",
            "dlayerfield1": "L",
            ...
            "dlayerlegend2": "Businesses",
            "dlayerdata2": "http://n00.cs.fiu.edu/cgi-bin/arquery.cgi?category=iypages_demographics&vid=&tfaction=shortdisplayflash&filetype=.xml",
            "dlayerselect2": "false",
            "dlayerfield2": "L",
            ...
            "dlayerlegend3": "Restaurants",
            "dlayerdata3": "http://n00.cs.fiu.edu/cgi-bin/arquery.cgi?category=us_companies_2013&tfaction=shortdisplayflash&arcriteria=1&Industry=Eating&filetype=.xml",
            "dlayerselect3": "true",
            "dlayerfield3": "L",
            "dlayercolor3": "#7fb2ff,0",

            "dLayerMarkerStyle4": [{
                "round_rect": true, zindex: 1, round_rect_width: 24, round_rect_height: 30, round_rect_radius: 6, line: true, line_color: "#bfbfef", line_width: 2, fill: true, fill_color: "#fff", fill_opacity: 70,
                icon_anchor: [0.5, 0.88], snaptopixel: true
            }, {
                "icon": "true", "icon_url": "./image/business.gif", "zindex": 2, scale: 1,
                icon_anchor: [0.5, 1], snaptopixel: true
            }
            ],
            "dLayerMarkerHoverStyle4": [{
                "round_rect": true, zindex: 1, round_rect_width: 24, round_rect_height: 30, round_rect_radius: 6, line: true, line_color: "#bfbfef", line_width: 2, fill: true, fill_color: "#fff", fill_opacity: 100,
                icon_anchor: [0.5, 0.88], snaptopixel: true
            }, {
                "icon": "true", "icon_url": "./image/business.gif", "zindex": 4, scale: 1,
                icon_anchor: [0.5, 1], snaptopixel: true
            }, {
                "marker": "true", "font_height": 14, "font_color": "#ffe", "border_color": "#000", "border_width": 2, "zindex": 4,
                "line_width": 2, "line_color": "#009",
            ...
            "dlayerlegend4": "Stores",
            "dlayerdata4": "http://n00.cs.fiu.edu/cgi-bin/arquery.cgi?category=us_companies_2013&tfaction=shortdisplayflash&arcriteria=1&SIC_CODE_Description|=store+stores&filetype=.xml",
            "dlayerselect4": "true",
            "dlayerfield4": "L",
            "dlayercolor4": "#fbffep,0",
